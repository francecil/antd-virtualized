...
export default class TreeStore {
  ...
  private setUnloadSelected(
    ...
    } else if (this.unloadSelectedKey === key) {
  ...
  clearSelected(): void {
    if (this.currentSelectedKey) {
      // eslint-disable-next-line
      this.mapData[this.currentSelectedKey] && this.setSelected(this.currentSelectedKey, false);
    }
  }
  ...
  setExpand(
    ...
    if (!node.isLeaf) {
      if (typeof this.options.load === 'function') {
        ...
        if (node._loading) return; // 正在加载的节点，直接返回
      }
      ...
      while (queue.length) {
        ...
        if (queue[0]._filterVisible === false) {
          ...
          queue[0].visible =
            queue[0]._parent === null || (queue[0]._parent!.expand && queue[0]._parent!.visible);
        }
        queue.shift();
      }
  ...
  insertBefore(
    ...
    this.insertIntoStore(node, parentNode, childIndex, flatIndex);
    this.emit('visible-data-change');
    return node;
  }
  ...
  private insertIntoStore(
    ...
    // eslint-disable-next-line
    nodes.forEach(
      childNode => (childNode._level = childNode._parent ? childNode._parent._level + 1 : 0),
    );
  ...
  filter(keyword: string, filterMethod: FilterFunctionType): void {
    ...
    this.flatData.forEach(node => {
      node._filterVisible =
        (node._parent && node._parent._filterVisible) || filterMethod(keyword, node);
    ...
    filterVisibleNodes.forEach(node => {
      ...
      while (parent) {
        ...
        this.setExpand(parent[this.options.keyField], true, false, false, false);
  ...
  private setUnloadCheckedKeys(keys: TreeNodeKeyType[]): void {
    ...
    const { length } = this.unloadCheckedKeys;
    ...
    if (
      newCheckedKeys.length === checkedKeysCache.length &&
      newCheckedKeys.every(val =>
        (checkedKeysCache as TreeNodeKeyType[]).some(cache => cache === val),
      )
    )
      return;
  ...
  private setUnloadSelectedKey(key: TreeNodeKeyType): void {
    ...
    if (this.mapData[key]) {
      ...
      // 节点未加载
      if (this.currentSelectedKey) {
        this.setSelected(this.currentSelectedKey, false, false);
      }
      this.unloadSelectedKey = key;
    }
  ...
  on<T extends keyof IEventNames>(
    ...
    listeners.forEach(lis => {
      if (this.listenersMap[eventName].indexOf(lis) === -1) {
        this.listenersMap[eventName].push(lis);
      }
    });
  }
  ...
  emit<T extends keyof IEventNames>(eventName: T, ...args: Parameters<IEventNames[T]>): void {
    if (!this.listenersMap[eventName]) return;
    const { length } = this.listenersMap[eventName];
